  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <h3
          className="text-lg font-bold text-primary mb-2 line-clamp-2 leading-snug cursor-pointer hover:text-secondary transition-colors duration-300"
          onClick={() => onViewClick && onViewClick(offer)}
        >
          {offer.title}
        </h3>

        <p className="text-primary/70 text-sm mb-5 line-clamp-2 flex-1 leading-relaxed">{offer.description}</p>

        <div className="pt-4 border-t border-primary/10 mt-auto flex justify-between items-end">
          <div className="space-y-1.5 flex-1 mr-4">
            {shopName && (
              <div className="font-bold text-primary/90 text-sm">{shopName}</div>
            )}
            <div className="flex items-center text-primary/60 text-xs font-medium">
            ...
            {offer.offerPrice && (
              <div className="flex items-center gap-2 mt-1">
                ...
                {offer.originalPrice && (
                  <span className="text-xs text-primary/50 line-through decoration-primary/30">₹{offer.originalPrice}</span>
                )}
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="bg-white border border-primary/20 hover:bg-primary hover:text-white text-primary text-xs font-bold px-3 py-2 rounded-full transition-colors whitespace-nowrap shadow-sm"
            >
              Visit Shop
            </Link>
            <button
