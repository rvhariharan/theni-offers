    ...
    <div className="group bg-white rounded-3xl shadow-soft hover:shadow-hover transition-all duration-500 flex flex-col h-full relative overflow-visible">
        ...
        <div className="absolute bottom-4 left-4 right-4 flex items-end gap-3 z-20">
             ...
             <div className="flex-1 pb-1">
                ...
                <h3 className="text-lg font-bold text-white leading-tight drop-shadow-md flex items-center gap-1">
                    {shop.name}
                    {shop.isVerified && <BadgeCheck size={16} className="text-blue-400 fill-blue-900/40 inline shrink-0" />}
                </h3>
             </div>
        </div>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col bg-white rounded-b-3xl -mt-0.5 relative z-10 border-t border-transparent">
        <div className="flex justify-between items-center mb-4">
             <div className="flex items-center bg-green-50 px-2.5 py-1 rounded-lg text-green-700 text-xs font-bold border border-green-100">
        ...
        <div className="text-sm text-gray-500 space-y-3 mb-6 flex-1">
          <div className="flex items-start">
            <MapPin size={16} className="mt-0.5 mr-3 text-primary/70 flex-shrink-0" />
            <span className="line-clamp-2 leading-relaxed text-gray-600">{shop.address}, {shop.area}</span>
          </div>
          <div className="flex items-center">
            <Phone size={16} className="mr-3 text-primary/70" />
        ...
        <Link 
          to={`/shops/${shop.id}`}
          className="group/btn flex items-center justify-center w-full bg-gray-50 hover:bg-gray-900 text-gray-900 hover:text-white py-3.5 rounded-2xl text-sm font-bold transition-all duration-300 shadow-sm hover:shadow-lg"
        >
