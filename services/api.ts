...
import { Offer, Shop, Job, Ad, FilterState } from '../types';
...
export const api = {
  getOffers: async (filters?: FilterState): Promise<Offer[]> => {
    await delay(300);
  ...
  getShops: async (filters?: FilterState): Promise<Shop[]> => {
    await delay(300);
  ...
  getJobs: async (filters?: FilterState): Promise<Job[]> => {
    await delay(300);
  ...
  getAds: async (placement: Ad['placement']): Promise<Ad[]> => {
    await delay(100);
    return mockAds.filter(a => a.placement === placement && a.isActive);
  },

  submitContact: async (data: any): Promise<{success: boolean}> => {
    await delay(800);
